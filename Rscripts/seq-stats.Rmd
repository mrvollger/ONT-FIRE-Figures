...
z=best.df %>%
    ...
    geom_text(
        ...
        aes(x=stat, y=5000, label=comma(stat)),
        size=2,
    ) +
...
my_ggsave("Figures/accuracy.pdf", height=3, width=4)
```
...
z=best.df %>%
    ...
    geom_label_repel(
        data=n50.df,
        aes(x=N50, y=0, label=comma(N50), color=name),
        size=2,
        alpha=0.75,
    ...
    theme(legend.position="top")
my_ggsave("Figures/read-length.pdf", height=3, width=4)
```
